...
import SkillChip from './comp/SkillChip';
      ...
      <Stack direction={'row'} flexWrap={'wrap'} gap={1}>
        {
          skillsData.map((item, index) => (
            ...
            <SkillChip key={index} label={item.label} icon={item.icon} />
          ))
        }
      </Stack>
    </Box>
    ...
    <Box id='skills'>
      ...
      <Grid2 container spacing={2} mt={2}>
        ...
        <Grid2 size={{ xs: 12, sm: 4 }} order={{ xs: 1, sm: 2 }} display="flex" justifyContent="center" alignItems="center">
          <Box component={'img'} loading='lazy' src='/images/cartoonChar.png' sx={{ width: { xs: '300px', sm: '100%' } }} />
        </Grid2>
